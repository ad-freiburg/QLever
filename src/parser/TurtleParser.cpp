...
bool TurtleParser<T>::pnameLN() {
  ...
  // HACK2(Hannah, OLD): Skip \-escaped matches. Note that because
  // view[pos] == ':', we have posEnd > 0 if posEnd != npos
  // while (posEnd != string::npos && view[posEnd - 1] == '\\') {
  //   posEnd = view.find_first_of(" \n,;", posEnd + 1);
  // }

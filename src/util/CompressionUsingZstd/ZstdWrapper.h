...
class ZstdWrapper {
 public:
  ...
  static void compressInChunks(char* src, size_t numBytes,
    ...
    ad_utility::WorkQueue<CompressedInput, true, decltype(getIndex)> writeQueue{
        100};
    ...
    for (size_t i = 0; i < numThreads - 1; ++i) {
      compressors.emplace_back([&] {
        while (auto optionalSpan = toCompressQueue.pop()) {
          ...
          auto writeToResult = [&](auto result) {
            compressedResult = std::move(result);
          };
          compress(optionalSpan->chunk.data(), optionalSpan->chunk.size(),
                   writeToResult, compressionLevel);
          writeQueue.push(CompressedInput{std::move(compressedResult),
                                          optionalSpan->index});
        }
      });
    }
    ...
    // The compression is complete, so all the indices should be in the
    // writeQueu
    writeQueue.finish();

    // The destructor of the writing thread blocks here until everything has
    // written.
  }
  ...
  requires(std::is_trivially_copyable_v<T>) static size_t
      decompressToBuffer(void* src, size_t numBytes, T* buffer,
